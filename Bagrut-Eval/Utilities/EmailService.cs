...
namespace Bagrut_Eval.Utilities
{
    ...
    public interface IEmailService
    {
        ...
        Task SendWelcomeEmailAsync(string toEmail, string firstName, string password, string subjectTitle);
    ...
    public class EmailService : IEmailService
    {
        ...
        public async Task SendWelcomeEmailAsync(string toEmail, string firstName, string password, string subjectTitle)
        {
            var templateFile = Path.Combine(_templatePath, "NewUserWelcomeTemplate.html");
            var baseTemplate = await File.ReadAllTextAsync(templateFile);

            var subject = "ברוכים הבאים למערכת תמיכת מעריכים";
            var personalizedBody = baseTemplate.Replace("{FirstName}", firstName).Replace("{Password}", password).Replace("{Subject}", subjectTitle);
        ...
        public async Task SendAddBulkEmailAsync(string email, string firstName, string added, string notAdded)
        {
            ...
            var subject = "הוספת משתמשים חדשים";
