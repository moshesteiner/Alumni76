...
namespace Bagrut_Eval.Pages
{
    ...
    public class UpdatePageModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            var originalPhone = UserToDisplay.Phone;
            ...
            {
                Phone = $"{cleanPhone.Substring(0, 2)}-{cleanPhone.Substring(2)}";
            }
            ...
            try
            {
                ...
                {
                    UserId = UserToDisplay.Id, // The ID of the user whose profile was affected
                    InitiatorId = initiatorId, // The ID of the user who performed the action (self-update)
                    Date = DateTime.UtcNow,    // Use 'Date' property as per your UserLog model                        
                    Description = $"עדכון פרופיל: שם: '{UserToDisplay.FirstName} {UserToDisplay.LastName}'. " +
                                  $"אימייל: '{UserToDisplay.Email}'. " // Use originalEmailFromDb for description
                                                                                                  //+ (passwordChanged ? "סיסמה שונתה." : "סיסמה לא שונתה.")

                };
                await _dbContext.UsersLog.AddAsync(logEntry);
